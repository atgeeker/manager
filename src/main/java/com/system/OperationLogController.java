...
import com.zotherdemo.page.PageInfo;
...
public class OperationLogController {
    ...
    public String queryLogPage(ModelMap modelMap,HttpServletRequest request) {
        PageInfo<SystemLog> pageInfo = new PageInfo<SystemLog>();

        int pageSize = pageInfo.getCountOfCurrentPage();//Integer.parseInt(request.getParameter("pageSize"));
        ...
        example.setOrderByClause("CREATE_TIME DESC");  //排序处理
        List<SystemLog> results = systemLogMapper.selectByExample(example);
        int totalCount = 220;
        pageInfo.setPageResults(results);
        pageInfo.setTotalCount(totalCount);

        modelMap.addAttribute("pageInfo", pageInfo);

        return "/system/systemman/log";
    }
